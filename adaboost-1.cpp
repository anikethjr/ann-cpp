...
#define ENSEMBLE_SIZE 5
#define COMMITTEE_SIZE 5
#define SAMPLING_FRACTION 0.27
...
void adaboost(vector<node*> ensemble,vector<map<string,string> > trainset,string target,vector<double> &alpha,int committeesize,set<string> discrete,set<string> continuous,vector<node*> &committee)
{
	vector<double> weights(trainset.size(),1.0);
	vector<vector<int> > classifications(ensemble.size(), vector<int>(trainset.size()));
	#pragma omp parallel for
		for(int i=0;i<ensemble.size();i++)
		{
			#pragma omp parallel for
				for(int j=0;j<trainset.size();j++)
					classifications[i][j] = (classify(trainset[j],target,ensemble[i],discrete,continuous));
		}
	for(int i=0;i<committeesize;i++)
	{
		double minwe = DBL_MAX;
		int classifier = -1;
		double em;
		...
		for(int j=0;j<ensemble.size();j++)
		{
			...
			{
				if(classifications[j][k] != stoi(trainset[k][target]))
					we += weights[k];
				w += weights[k];
			}
			if(we<minwe)
		...
		committee.push_back(ensemble[classifier]);
		double temp = exp(alpha[i]);
		//update weights
		#pragma omp parallel for
			for(int j=0;j<trainset.size();j++)
			{
				if(classifications[classifier][j] != stoi(trainset[j][target]))
					weights[j] = weights[j] * temp;
				else
					weights[j] = weights[j] * (1.0/temp); 
			}
		ensemble.erase(ensemble.begin() + classifier);
		classifications.erase(classifications.begin() + classifier);
	}
}
...
int main()
{
	...
	adaboost(ensemble,trainset,target,alpha,COMMITTEE_SIZE,discrete,continuous,committee);
