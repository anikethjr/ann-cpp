...
#define COMMITTEE_SIZE 5
#define SAMPLING_FRACTION 1
...
void populaterandom(vector<map<string,string> > &trainset, vector<map<string,string> > &randtrainset,vector<double> probabilitydistribution) //populates random training examples for training the decision trees based on the supplied probability distribution
{
	rng.seed(random_device()());
    discrete_distribution<int> dist(probabilitydistribution.begin(),probabilitydistribution.end());

    int iter = ceil((double)trainset.size()*SAMPLING_FRACTION);
	for(int i=0;i<iter;i++) //sampling from the training set
		randtrainset[i] = trainset[dist(rng)];
}

node* id3(set<string> discrete, set<string> continuous,vector<map<string,string> > trainset,string target)
{
	if(trainset.size() == 0)
	{
		node *neg = new node;
	...
	if(discrete.find(minentattribute)!=discrete.end())//the decision attribute is discrete valued
	{
		...
		for(map<string,vector<map<string,string> > >::iterator it = m.begin();it!=m.end();++it)
		{
			...
			decisionnode->children[it->first] = id3(discrete,continuous,it->second,target);
		}
	}
...
void adaboost(vector<map<string,string> > &trainset,string &target,vector<double> &alpha,int committeesize,set<string> &discrete,set<string> &continuous,vector<node*> &committee,vector<string> &attributelist)
{
	vector<double> weights(trainset.size(),1.0);
	vector<int> classifications(trainset.size());
	for(int i=0;i<committeesize;i++)
	{
		node *tree;
		vector<map<string,string> > randtrainset(ceil((double)trainset.size()*SAMPLING_FRACTION)); 	
	    populaterandom(trainset,randtrainset,weights);
	    tree = id3(discrete,continuous,randtrainset,target);
	    cout<<"Decision tree "<<i+1<<" trained"<<endl;
		for(int j=0;j<trainset.size();j++)
			classifications[j] = (classify(trainset[j],target,tree,discrete,continuous));
		double em;
		double we = 0;
		double w = 0;
		//find tree which minimizes weighted error				
		for(int k=0;k<trainset.size();k++)
		{
			if(classifications[k] != stoi(trainset[k][target]))
				we += weights[k];
			w += weights[k];
		}
		em = we/w;
		alpha[i] = 0.5 * log((1.0-em)/em);			
		committee.push_back(tree);
		double temp = exp(alpha[i]);
		//update weights
		for(int j=0;j<trainset.size();j++)
		{
			if(classifications[j] != stoi(trainset[j][target]))
				weights[j] = weights[j] * temp;
			else
				weights[j] = weights[j] * (1.0/temp); 
		}
	}
}
...
int main()
{
	...
	vector<double> weights(trainset.size(),1.0);
	cout<<"AdaBoost has been initiated.....\n";
	vector<double> alpha(COMMITTEE_SIZE);
	vector<node*> committee;
	adaboost(trainset,target,alpha,COMMITTEE_SIZE,discrete,continuous,committee,attributelist);
	cout<<"AdaBoost has been completed.....\n";
	...
	for(int i=0;i<TESTING_SIZE;i++)
	{
		...
		double answer = 0.0;
		vector<double> classifications(committee.size());
		for(int j=0;j<committee.size();j++)
			classifications[j] = alpha[j]*classify(temp,target,committee[j],discrete,continuous);
		for(int j=0;j<committee.size();j++)
			answer += classifications[j];
		if(round(answer) == stoi(temp[target]))
			correct++;
		total++;
	}
